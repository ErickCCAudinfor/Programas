...
' PORTADO DE ActualizaPrecios sin reescribir la lógica: es la razón por la que la
' solución es mixta C#/VB. Cambios respecto al original, solo los imprescindibles:
'   · Imports DocumentFormat.OpenXml.Drawing y SigeCom.Repository: eliminados, no se
'     usaba ningún tipo de ellos y arrastraban paquetes enteros.
'   · System.Data.SqlClient -> Microsoft.Data.SqlClient, que es el proveedor mantenido.
'     Ojo: cifra la conexión por defecto, y por eso las cadenas llevan
'     TrustServerCertificate=True (ver RepositorioEntornos).
'   * Las rutas fijas al escritorio se han cambiado por RutasSalida.Asegurar(...),
'     que resuelve el escritorio real del usuario y lo centraliza en ConsultasBO.
'     Es el unico cambio de logica de este fichero.
...
where ct.idcontratotarifa in ({CodTJoin}) order by t.IdTarifa"
            ...
            Dim rutaCarpeta As String = SigeGestor.Core.Configuracion.RutasSalida.Asegurar("Precios")
...
group by IdContratoTarifa,tg.TextoTarifaGrupo,tg.IdTarifaGrupo
            ...
            Dim rutaCarpeta As String = SigeGestor.Core.Configuracion.RutasSalida.Asegurar("PreciosPersonalizados")
