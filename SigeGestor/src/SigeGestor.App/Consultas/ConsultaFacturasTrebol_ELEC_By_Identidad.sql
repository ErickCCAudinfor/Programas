...
,FacturasVentaConsulta AS (
	...
	inner join contrato on fvc.CodigoContrato = contrato.CodigoContrato
	inner join cliente cl on contrato.IdCliente= cl.IdCliente
    WHERE 
	fvc.Entorno='E1' and
	fvc.SerieFactura IS NOT NULL 
	 --fvc.FechaFactura >= '01/04/2026' and fvc.FechaFactura <= '02/04/2026'
	 and identidad in (
'identidadReplace'
	 )
	 --and IdFacturaVentaCabecera=775318
)
,
ContratoPot as(
select 
