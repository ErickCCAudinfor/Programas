--rick _Gestor Sige_Consulta Cuentas LB2B
;WITH ContratosAgente AS (
    SELECT CodigoContrato
    FROM Contrato WITH (NOLOCK)
    WHERE nombreagente like'%NombreAgenteReplace%'  AND Entorno = 'E1' 
)
...
,FacturasVentaConsulta AS (
    ...
    INNER JOIN ContratosAgente ca WITH (NOLOCK) ON fvc.CodigoContrato = ca.CodigoContrato
    WHERE 
	fvc.Entorno='E1' and
	fvc.SerieFactura IS NOT NULL AND
	 fvc.FechaFactura >= 'DesdeFechaReplace' and fvc.FechaFactura <= 'hastaFechaReplace'
...
Agente.CodigoTipoAgente,
       case when Agente.CodigoTipoAgente = 3 then Agente.NombreAgente
            else null
       end as SubAgente,
       case when Agente.CodigoTipoAgente = 2 then Agente.NombreAgente 
            when Agente.CodigoTipoAgente = 3 then Agenteb.NombreAgente 
            else null
       end as Agente,
       case when Agente.CodigoTipoAgente = 1 then Agente.NombreAgente 
            when Agente.CodigoTipoAgente = 2 then Agenteb.NombreAgente 
            when Agente.CodigoTipoAgente = 3 then Agentec.NombreAgente 
            else null
       end as DistribuidorAgente
...
inner join CUPS with (nolock) on c.idcups = cups.idcups
      left join Agente with (nolock) on Agente.IdAgente = c.IdAgente
         left join (select IdAgente, NombreAgente, IdAgenteNivelAnterior from Agente) as Agenteb on Agente.IdAgenteNivelAnterior = Agenteb.IdAgente
         left join (select IdAgente, NombreAgente, IdAgenteNivelAnterior from Agente) as Agentec on Agenteb.IdAgenteNivelAnterior = Agentec.IdAgente
